...
class SL_Matcher
{
	...
	public static function init_matching(): array
	{
		...
		// Filter IDs based on category settings
		$filtered_ids = SL_Indexer::get_filtered_post_ids();
		if (!empty($filtered_ids)) {
			// Intersect to ensure we only process existing indexed posts that are also allowed
			$source_ids = array_values(array_intersect($source_ids, $filtered_ids));
		}
		...
		// Filter targets based on category settings
		if (!empty($filtered_ids)) {
			$title_rows = array_filter($title_rows, function ($row) use ($filtered_ids) {
				return in_array($row->post_id, $filtered_ids);
			});
		}
		...
		if (empty($batch_ids)) {
			...
			if ($progress['gemini_enabled'] && !empty($progress['candidates'])) {
				...
				foreach ($progress['candidates'] as $candidate) {
					...
					if (
						isset($anchor_url_map[$anchor_normalized]) &&
	...
	private static function process_gemini_batch(array $progress): array
	{
		...
		foreach ($batch as $candidate) {
			...
			if (
				isset($batch_saved_anchors[$anchor_normalized]) &&
