...
class SL_Indexer
{
	...
	/**
	 * Get list of all post IDs that match the configured post types and category filters.
	 *
	 * @return int[]
	 */
	public static function get_filtered_post_ids(): array
	{
		$post_types = SL_Settings::get('post_types', ['post']);
		$post_type_terms = SL_Settings::get('post_type_terms', []);
		$all_ids = [];
		...
		// If no terms are selected for any type, revert to simple global query (faster)
		$has_filters = false;
		foreach ($post_types as $pt) {
			if (!empty($post_type_terms[$pt])) {
				$has_filters = true;
				break;
			}
		}
		...
		// Complex filtering: query each post type individually
		foreach ($post_types as $pt) {
			$args = [
				'post_type' => $pt,
				'post_status' => 'publish',
			...
			$tax_queries = [];
			if (!empty($post_type_terms[$pt])) {
				foreach ($post_type_terms[$pt] as $tax => $terms) {
					if (!empty($terms)) {
						$tax_queries[] = [
							'taxonomy' => $tax,
							'field' => 'term_id',
	...
	public static function init_batch(): array
	{
		...
		// Get all posts to process (respecting category filters)
		// We use a helper to get specific IDs if categories are selected.
		$posts = self::get_filtered_post_ids();
		...
		$progress = [
			'total_posts' => $total,
			'ids' => $posts,      // Store exact list of IDs to process
			'processed' => 0,
	...
	public static function process_batch(): array
	{
		...
		// Get batch of posts from stored IDs
		$all_ids = $progress['ids'] ?? [];
		$batch_ids = array_slice($all_ids, $progress['offset'], self::AJAX_BATCH_SIZE);
	...
	public function run(): void
	{
		...
		// 1. Fetch recent posts (fetch more than BATCH to allow for filtering)
		$recent_posts = get_posts([
			'post_type' => $post_types,
			'post_status' => 'publish',
			'numberposts' => self::BATCH_POSTS * 3, // Fetch extra
			'orderby' => 'modified',
			'order' => 'DESC',
			'update_post_meta_cache' => false,
			'update_post_term_cache' => false,
		...
		// 2. Filter against allowed IDs (categories)
		$allowed_ids = self::get_filtered_post_ids();
		$posts = [];

		foreach ($recent_posts as $p) {
			if (in_array($p->ID, $allowed_ids)) {
				$posts[] = $p;
			}
		}

		// Limit to batch size
		$posts = array_slice($posts, 0, self::BATCH_POSTS);
